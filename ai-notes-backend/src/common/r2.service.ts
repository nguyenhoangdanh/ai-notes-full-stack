...
export class CloudflareR2Service {
  ...
  async uploadFile(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.logger.log(`🔄 R2 file upload attempt ${attempt}/${maxRetries}, filename: ${fileName}`);
        
        // Use different approach from avatar upload - direct S3 API
        const path = `/${this.bucketName}/${fileName}`;
        ...
        if (response.ok) {
          const publicUrl = `${this.publicUrl}/${fileName}`;
          this.logger.log(`✅ File uploaded to R2 successfully: ${publicUrl}`);
          return publicUrl;
        } else {
          const errorText = await response.text().catch(() => 'Unable to read response body');
          const errorMsg = `R2 API Error ${response.status}: ${response.statusText} - ${errorText}`;
        ...
        this.logger.error(`❌ R2 file upload attempt ${attempt}/${maxRetries} failed:`, {
          message: error.message,
          cause: error.cause?.message || error.cause,
          code: error.code,
        });
        ...
        if (attempt < maxRetries) {
          ...
          this.logger.warn(`⏳ Retrying R2 file upload in ${delay}ms...`);
          await new Promise(resolve => setTimeout(resolve, delay));
          continue;
        }
  ...
  async deleteFile(fileUrl: string): Promise<void> {
    try {
      ...
      if (!fileName) {
        this.logger.warn(`Could not extract filename from URL: ${fileUrl}`);
        return;
      }

      this.logger.log(`🗑️ Starting file deletion: ${fileName}`);
      ...
      if (response.ok || response.status === 404) {
        this.logger.log(`✅ R2 file deleted successfully: ${fileName}`);
      } else {
